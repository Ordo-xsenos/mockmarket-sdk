...
Usage:
    MOCKMARKET_API_KEY=mk_... python example_arena_bot.py
"""
...
from mockmarket import EngineSandboxCreate, MockMarketAsyncClient
...
async def main() -> None:
    async with MockMarketAsyncClient(API_KEY, base_url=BASE_URL) as client:
        sb = await client.arena.create_sandbox(
            EngineSandboxCreate(challenge_preset=PRESET, agent_name="arena-example")
        )
        await sb.start()
        ...
        for i, e in enumerate(await client.arena.leaderboard(preset=PRESET, limit=5), 1):
